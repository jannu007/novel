 ...
 * といった隙が残る。そこで、語り部が一度も使わない「外へ送る道具」を
 * 起動直後に取り上げてしまう。CSPが効かない場所でも、道具が無ければ送れない。
 *
 * 語り部はこれらを1か所も使っていないので、取り上げても機能は何も減らない。
 * （サービスワーカーの中は別の世界なので影響を受けない。オフライン起動は従来どおり動く。）
 *
 * なお、朗読アプリで本当に危ないのは通信の道具ではなく「声」のほうである。
 * 声を出すしくみ（音声合成）には、本文をメーカーのサーバーへ送って
 * 読み上げる種類のものがあり、これはCSPの外側で動く。その対策は
 * `src/voice/speech.ts` にある（端末の中だけで話す声しか使わない）。
 */
...
function refuse(name: string): never {
  throw new Error(`語り部は通信しません（${name} は使えないようにしてあります）`);
}
